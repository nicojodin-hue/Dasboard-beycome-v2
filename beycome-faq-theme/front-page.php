        ...
        <div class="bc-resources-grid">
            ...
            <div>
                ...
                if ($rt['type'] === 'category') {
                    ...
                    if ($cat_obj) echo '<a href="' . esc_url(get_category_link($cat_obj->term_id)) . '" style="font-size:var(--text-sm);font-weight:700;color:var(--c-accent-orange);display:block;margin-top:8px">Browse all ' . esc_html($rt['name']) . ' articles &rarr;</a>';
                } else {
                    $tag_obj = get_term_by('slug', $rt['tag'], 'post_tag');
                    if ($tag_obj) echo '<a href="' . esc_url(get_tag_link($tag_obj->term_id)) . '" style="font-size:var(--text-sm);font-weight:700;color:var(--c-accent-orange);display:block;margin-top:8px">Browse all ' . esc_html($rt['name']) . ' articles &rarr;</a>';
                }
                ?>
            </div>
            <?php endforeach; ?>
